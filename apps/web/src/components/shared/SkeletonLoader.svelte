			...
			<div class="space-y-3">
				{#each Array(lines) as _}
					<div class="flex items-center gap-3 rounded-lg bg-gray-900 p-3">
						<div class="h-4 w-8 rounded bg-gray-800"></div>
						<div class="h-10 w-10 rounded-full bg-gray-800"></div>
						<div class="h-4 flex-1 rounded bg-gray-800"></div>
					</div>
				{/each}
			</div>
			<div class="rounded-xl border border-gray-700 bg-gray-900 p-4">
				<div class="mb-3 h-5 w-32 rounded bg-gray-800"></div>
				<div class="h-64 rounded bg-gray-800/50"></div>
			</div>
		</div>
	{:else}
		{#each Array(lines) as _}
