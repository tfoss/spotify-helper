...
<div class="flex flex-col items-center justify-center py-20">
	<h1 class="mb-4 text-4xl font-bold text-green-600 dark:text-green-400">Spotify Helper</h1>
	...
	{:else if initError}
		<div class="rounded-xl border border-red-300 bg-red-50 p-6 text-center dark:border-red-800 dark:bg-red-950/50">
			<p class="text-lg font-semibold text-red-600 dark:text-red-400">Failed to initialize</p>
	...
	{:else if $authStore.isAuthenticated}
		...
		{#if showWipeConfirm}
			<div class="mt-3 rounded-lg border border-red-300 bg-red-50 p-4 text-center dark:border-red-800 dark:bg-red-950/50">
				<p class="text-sm text-red-600 dark:text-red-400">This will delete all cached data. Continue?</p>
	...
	{:else}
		...
		{#if $authStore.error}
			<p class="mt-4 text-sm text-red-400 dark:text-red-400">{$authStore.error}</p>
		{/if}
	{/if}
</div>
